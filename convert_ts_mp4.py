...
for i in prefijos:
    ...
    nombre_archivo_video = nombre_archivo_video.replace(" - Curso Profesional de Typescript-playlist", "")
    archivo_salida = os.path.join(carpeta_salida_fusion, f"{nombre_archivo_video}.mp4")
    # invocamos a ffmpeg para la fusion
    print("\033[32m" + f"Procesando: {archivo_video} -> {archivo_salida}" + "\033[0m")
    subprocess.run(f"{ruta_ffmpeg} -i \"{archivo_video}\" -c:v libx264 \"{archivo_salida}\"")
